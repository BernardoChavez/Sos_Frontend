    ...
    <div class="p-6 max-w-4xl mx-auto h-full flex flex-col animate-in">
      <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 shadow-2xl flex-1 flex flex-col overflow-hidden">
        ...
        @if (trackingData && trackingData.estado !== 'completado') {
          ...
          <!-- IA Diagnosis Card con Scroll (FIX) -->
          <div class="bg-blue-600/10 border border-blue-500/20 rounded-[2.5rem] p-6 mb-6">
             <div class="flex items-center gap-2 mb-4 text-blue-400 font-black text-[10px] uppercase tracking-widest">
               <i class="bi bi-robot"></i> Ficha Técnica IA
             </div>
             <p class="text-white font-black text-lg tracking-tighter italic mb-4">{{trackingData.resumen_ia || 'Analizando incidente...'}}</p>
             
             <!-- Contenedor con Scroll para el diagnóstico largo -->
             <div *ngIf="trackingData.resumen_ia" class="max-h-40 overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-blue-500/20">
                <p class="text-slate-400 text-xs leading-relaxed whitespace-pre-wrap italic">
                    "{{trackingData.transcripcion_voz_ia || 'Escuchando reporte...'}}"
                </p>
             </div>
          </div>
        }
