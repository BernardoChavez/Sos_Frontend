      ...
      <header class="mb-12">
        <div class="flex flex-col md:flex-row justify-between items-end gap-6">
          ...
          <button (click)="cargarHistorial()" class="w-12 h-12 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center">
              <i class="bi bi-arrow-clockwise text-blue-600 text-xl"></i>
          </button>
        </div>
      </header>
      ...
      <div *ngIf="mostrarReporte && reporteSeleccionado" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 animate-in">
                ...
                <div class="p-10 space-y-6">
                    ...
                    <div class="flex justify-between border-b border-slate-100 pb-4">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cliente</span>
                        <span class="font-bold text-slate-800">{{reporteSeleccionado.cliente}}</span>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-4">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Vehículo</span>
                        <span class="font-bold text-slate-800">{{reporteSeleccionado.vehiculo}}</span>
                    </div>
                    <div>
                    ...
                    <div class="bg-blue-50 p-6 rounded-2xl flex justify-between items-center mb-2">
                    ...
                    <!-- Detalles del Pago -->
                    <div class="px-6 py-2 flex flex-col gap-2 bg-slate-50 rounded-xl border border-slate-100">
                        <div class="flex justify-between items-center">
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Método</span>
                            <span class="text-[10px] font-black text-slate-700 uppercase">{{reporteSeleccionado.metodo_pago}}</span>
                        </div>
                        <div class="flex justify-between items-center" *ngIf="reporteSeleccionado.monto_recibido">
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Efectivo Recibido</span>
                            <span class="text-[10px] font-black text-slate-700">Bs. {{reporteSeleccionado.monto_recibido | number:'1.2-2'}}</span>
                        </div>
                        <div class="flex justify-between items-center" *ngIf="reporteSeleccionado.cambio !== null && reporteSeleccionado.cambio !== undefined">
                            <span class="text-[9px] font-black text-green-600 uppercase tracking-widest">Cambio Entregado</span>
                            <span class="text-[10px] font-black text-green-600">Bs. {{reporteSeleccionado.cambio | number:'1.2-2'}}</span>
                        </div>
                    </div>
...
export class HistorialComponent implements OnInit {
  ...
  imprimirReporte() {
    ...
    const html = `
      ...
      <div class="row"><span class="label">Método de Pago</span><span class="value">${r.metodo_pago}</span></div>
      
      ${r.monto_recibido ? `
      <div class="row" style="background: #f0fdf4; border-bottom: none;"><span class="label" style="color: #166534;">Efectivo Recibido</span><span class="value" style="color: #166534;">Bs. ${r.monto_recibido.toFixed(2)}</span></div>
      <div class="row" style="background: #f0fdf4;"><span class="label" style="color: #166534;">Cambio Entregado</span><span class="value" style="color: #166534;">Bs. ${r.cambio.toFixed(2)}</span></div>
      ` : ''}
