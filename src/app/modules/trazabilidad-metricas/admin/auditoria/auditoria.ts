                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr *ngFor="let log of logs" class="hover:bg-blue-50/30 transition-all text-sm group">
                            ...
                            <td class="px-8 py-6 text-slate-600 font-medium text-xs">{{ log.inicio }}</td>
                            <td class="px-8 py-6">
                                <span *ngIf="log.cierre === 'Sesión Activa'" class="inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">
                                    <span class="w-1.5 h-1.5 bg-green-500 rounded-full"></span> Activo
                                </span>
                                <span *ngIf="log.cierre !== 'Sesión Activa'" class="text-slate-400 font-medium text-xs">
                                    {{ log.cierre }}
                                </span>
                            </td>
                        </tr>
